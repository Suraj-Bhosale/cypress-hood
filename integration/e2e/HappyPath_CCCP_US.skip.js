...
import sat_steps from '../../utils/HappyPath_steps/CCCP_US_HappyPath/satelite_lab_steps';
import m_steps from '../../utils/HappyPath_steps/CCCP_US_HappyPath/manufacturing_steps.js'
...
context('CCCP US Therapy Happy Path', () => {
  beforeEach(() => {
    cy.server();
  ...
  it('Collection flow', () => {
    ...
    col_steps.bagDataEntryDay(scope.coi, therapy);
    col_steps.collectionSummary(scope, therapy);
    col_steps.changeOfCustody(scope.coi, therapy);
    common.loginAs('phil');
    cy.visit('/collection');
    common.loadCollection();
    cy.get(`tr[data-testid="treatment-${scope.coi}"]`).click();
    col_steps.confirmChangeOfCustody(scope.coi);
    col_steps.collectionTransferProductToShipper(scope.coi, "ln-2-shipper-ship-bags-from-apheresis-site-to-satellite-lab-cccp", therapy);
  ...
  it('Satellite Lab flow', () => {
    ...
    sat_steps.satLabVerifyShipment('satellite-lab', '-cccp', scope.coi);
  ...
  it('Manufacturing flow', () => {
    ...
    m_steps.manufacturingVerifyShipper(scope.coi, 'satellite-lab', '-cccp');
  ...
  it('Infusion flow', () => {
    ...
    cy.get(".patient-id")
      .contains(scope.patientInformation.subjectNumber)
      .click();
    inf_steps.infusionReceiveShipment('manufacturing-site', '-cccp', scope.coi);
