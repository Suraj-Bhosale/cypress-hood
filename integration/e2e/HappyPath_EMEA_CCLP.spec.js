import order_steps from '../../utils/HappyPath_steps/EMEA_CCLP_HappyPath/ordering_steps_cilta';
import col_steps from '../../utils/HappyPath_steps/EMEA_CCLP_HappyPath/collection_steps_cilta';
import m_steps from '../../utils/HappyPath_steps/EMEA_CCLP_HappyPath/manufacturing_steps_cilta.js';
import inf_steps from '../../utils/HappyPath_steps/EMEA_CCLP_HappyPath/infusion_steps_cilta';
import sat_steps from '../../utils/HappyPath_steps/EMEA_CCLP_HappyPath/satelite_lab_steps_cilta'
...
context('EMEA CC Therapy Happy Path', () => {
  // beforeEach(() => {
  //   cy.server();
  ...
  // const scope = {};
  // const therapy = therapies.emea_cc;
  ...
  it('Create and Approvs an order through api', () => {
  //   common.loginAs('nina');
  //   cy.visit('/ordering');
  ...
  let mutationQuery = 'createApprovedOrderTransaction';
    ...
    makeGraphqlCypressRequest(
      orderMutationRequest(mutationQuery, mutationResponse, 'cilta-cel-jnj-emea-cclp'),
      orderVariables,
  ...
  it('Collection flow', () => {
    ...
    col_steps.patientVerificationEmeaCc(therapy, scope);
    col_steps.collectionBagIdentificationEmeaCc();
    col_steps.collectionInformationPrintingEmeaCc();
    col_steps.collectionProcedureInformation();
    col_steps.collectionSummaryEmeaCc(scope);

    common.loginAs('phil');
    cy.visit('/collection');
    common.loadCollection();
    cy.waitForElementAndClick(`tr[data-testid="treatment-${scope.coi}"]`);
    col_steps.transferProductToShipper(scope.coi, input.day1Bag1Udn, therapy.leg_name);
    col_steps.collectionShipmentChecklistEmeaCc(scope);
    col_steps.collectionShippingSummaryEmeaCc(scope);
  });
  ...
  it('Satellite Lab flow', () => {
    ...
    sat_steps.satLabVerifyShipment(scope.coi, therapy, 'from-satellite-lab', 'to-manufacturing-site');
    ...
    sat_steps.satLabCryopreservationLabelscclp(scope.coi, 'EMEA Cilta-Cel CC');
    ...
    sat_steps.satLabPrintShipperLabels(therapy, 'EMEA Cilta-Cel CC');
    sat_steps.satLabBagSelection();
    sat_steps.satLabTransferProductToShipper(scope.coi, therapy);
  ...
  it('Manufacturing flow', () => {
    ...
    m_steps.manufacturingVerifyShipper(scope.coi, therapy, 'from-satellite-lab', 'to-manufacturing-site');
    ...
    m_steps.manufacturingProductReceiptSummaryVerifyEmea(therapy, scope);
  ...
  it('Infusion flow', () => {
    ...
    cy.get('.patient-id')
      ...
      .click();
    //FIXME: input LN2 coi field not taking any of values
    inf_steps.infusionReceiveShipment('manufacturing-site', therapy, scope.coi);
    inf_steps.infusionShipmentReceiptChecklist(therapy, input.evoLast4Digits, input.tamperSealNumber, true);
