...
export default {
    ...
    satLabVerifyShipment: (coi, therapy, fromSite, toSite) => {
        cy.get(`[data-testid="ln-2-shipper-ship-bags-${fromSite}-${toSite}-${therapy}-1-input-field"]`).type(coi);
        cy.get(`[data-testid="ln-2-shipper-ship-bags-${fromSite}-${toSite}-${therapy}-1-action-trigger-button"]`).click();
        cy.wait('@labelVerifications');
        common.ClickPrimaryActionButton();
    },
    ...
    cryopreservationData: coi => {
        cy.wait(2000)
        cy.get('[data-testid="#/properties/shipping_receipt_checklist/properties/volume_1-input"]').type(inputs.volume);
        common.ClickPrimaryActionButton();
    },
