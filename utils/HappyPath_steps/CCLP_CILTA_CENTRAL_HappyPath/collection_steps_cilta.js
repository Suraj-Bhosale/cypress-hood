...
export default {
  ...
  collectionBagIdentification: () => {
    ...
    cy.get('[data-test-id="enter-and-confirm-block"] p:first')
    ...
    cy.get('[data-testid="input-enter-day_1_bag_1_udn"]')
      .first()
      .clear()
      .type(inputs.day1Bag1Udn);
    cy.get('[data-testid="input-confirm-day_1_bag_1_udn"]')
      .first()
      .clear()
      .type(inputs.confirmDay1Bag1Udn);
    cy.get("body").then($body => {
    ...
    cy.get('[data-testid="recordButton-day_1_bag_1_udn"]')
      .should('contain', assertions.record)
      .click();
    cy.wait('@createLabelScanValue');
    cy.get('[data-testid="green-checkmark-day_1_bag_1_udn"]').should('be.visible');
    cy.get('[data-testid="primary-button-action"]').contains(
      assertions.nextLabel
    );
  ...
  cryopreservationDataLocal: coi => {
    ...
    cy.wait('@labelVerifications');
    cy.get('[data-testid="#/properties/shipping_checklist/properties/volume_1-input"]').type(inputs.volume)
    cy.get('[id="#/properties/data/properties/is_confirmed"]').click()
    common.ClickPrimaryActionButton();
  },
  ...
  cryopreservationTransferProductToShipper: (coi, therapy) => {
    ...
    cy.get(`[data-testid="ln-2-shipper-ship-bags-from-apheresis-site-to-manufacturing-site-${therapy}-1-input-field"]`).type(coi)
    cy.get(`[data-testid="ln-2-shipper-ship-bags-from-apheresis-site-to-manufacturing-site-${therapy}-1-action-trigger-button"]`).click()
  ...
  transferProductToShipper: (coi, bag_identifier, therapy = 'us-cclp-cilta', fromSite = 'apheresis-site', toSite = 'satellite-lab') => {
    cy.get(`[data-testid="ln-2-shipper-ship-bags-from-${fromSite}-to-${toSite}-${therapy}-1-input-field"]`).type(coi)
    cy.get(`[data-testid="ln-2-shipper-ship-bags-from-${fromSite}-to-${toSite}-${therapy}-1-action-trigger-button"]`).click()
  ...
  collectionShipmentChecklistCELLEX: (scope, testId) => {
    ...
    cy.wait('@labelVerifications');
    cy.get('[data-testid="pass-button-apheresis_not_exposed"]').click();
    cy.get('[data-testid="pass-button-apheresis_bag_label"]').click();
    cy.get('[data-testid="pass-button-temperature_monitor_active"]').click();
