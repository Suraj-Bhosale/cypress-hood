...
export default {
	...
	cryopreservationTransferProductToShipperEmeaLc: (coi, therapy, fromSite, toSite) => {
		...
		if (Cypress.env('runWithTranslations')) { // Title
			...
			// labels
			translationHelpers.assertSingleField('[data-testid=question-text-case_intact_1]', collectionCiltaAssertion.transferToShipper.questionCaseLcdcqr)

			translationHelpers.assertSingleField('[data-testid=question-text-temp_out_of_range_1]', collectionCiltaAssertion.transferToShipper.questionTempOutofRangeLclpdcqr)
	...
	cryopreservationShipmentChecklist: scope => {
		if (Cypress.env('runWithTranslations')) {
			...
			// labels
			translationHelpers.assertSingleField('[data-test-id="label-step-row-description"]', collectionCiltaAssertion.shipmentChecklist.enterAirWaybilllcdcQr)
