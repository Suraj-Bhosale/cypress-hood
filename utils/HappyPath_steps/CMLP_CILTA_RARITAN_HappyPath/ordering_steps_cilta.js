...
export default {
  ...
  AddPatientInformation: (
    ...
    if (Cypress.env('runWithTranslations')) {
      ...
      if (therapy.category == 'cilta') {
        cy.log('Medical record number')
        //medical record number
        translationHelpers.assertSingleField('[id="#/properties/custom_fields/properties/medical_record_number"]', orderingAssertionsCilta.patientInformation.medicalRecordNumber);
      } else {
        // patient ID
    ...
    inputChecker.dropDownSelect('[data-testid="institution_id"] .select',input.prescriber_select_index);
    if (isEMEA || therapy.unique_id == '_emea_lc') {
      inputHelpers.inputSingleField('[data-testid="#/properties/custom_fields/properties/patient_id-input"]',patientData.patientId);
    }
    else {
    ...
    //   cy.get('[data-testid="#/properties/lot_number-input"]').type(patientData.lotNumber);
    // }
  ...
  schedulingCheckAvailability: (therapy, region = 'US') => {
    ...
    if (Cypress.env('runWithTranslations')) {
      ...
      if (therapy.has_sat_lab) {
        //site name
        translationHelpers.assertSingleField(`[data-testid=lbl-site-select-satellite_lab${therapy.scheduler_suffix}]`, orderingAssertionsCilta.scheduling.selectSite);
        //contact person
        translationHelpers.assertSingleField(`[data-testid=lbl-contact-person-satellite_lab${therapy.scheduler_suffix}]`, orderingAssertionsCilta.scheduling.contactPerson);
        //contact phone number
        translationHelpers.assertSingleField(`[data-testid=lbl-phone-number-satellite_lab${therapy.scheduler_suffix}]`, orderingAssertionsCilta.scheduling.phoneNumber);
        //additional notes
        translationHelpers.assertSingleField(`[data-testid=lbl-additional-notes-satellite_lab${therapy.scheduler_suffix}]`, orderingAssertionsCilta.scheduling.additionalNotes);
      }
  ...
  SubmitOrder: (scope, therapy) => {
    ...
    if (Cypress.env('runWithTranslations')) {
      ...
      if (therapy.has_sat_lab) {
        ...
        translationHelpers.assertChildElement('[data-test-id="ordering-summary-block"]', 'h3', orderingAssertionsCilta.confirmation.satLab, 4);

        translationHelpers.assertChildElement('[data-test-id="ordering-summary-block"]', 'h3', orderingAssertionsCilta.confirmation.drugProduct, 5);
      }
      else {
      ...
      if (therapy.unique_id == '_emea_lcdc' || therapy.unique_id == '_emea_lcdc_qr') {
        cy.log('Medical record number')
        //medical record number
        ...
        //medical record number
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 7, 'div', therapy.category == 'cilta' ? orderingAssertionsCilta.confirmation.medicalRecordNumber : orderingAssertionsCilta.confirmation.patientId);
        // patient ID
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 7, 'span', therapy.category == 'cilta' ? scope.patientInformation.medicalRecordNumber : scope.patientInformation.patientId);
      }
      //treatment ordering site
      ...
      if (therapy.has_sat_lab) {
        ...
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 15, 'span', inputs.cryoInstitution);
        ...
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 17, 'div', orderingAssertionsCilta.confirmation.dropoff);
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 17, 'span', inputs.dropOffAddress);

        //delivery contact
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 18, 'div', orderingAssertionsCilta.confirmation.deliveryContact);
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 18, 'span', inputs.Petya);

        //delivery number
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 19, 'div', orderingAssertionsCilta.confirmation.deliveryNumber);
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 19, 'span', inputs.number2);
        ...
        //delivery site
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 21, 'div', orderingAssertionsCilta.confirmation.deliverySite);
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 21, 'span', inputs.testCoe1);

        //delivery date
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 22, 'div', orderingAssertionsCilta.confirmation.deliveryDate);

        //street address
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 23, 'div', orderingAssertionsCilta.confirmation.streetAddress);
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 23, 'span', inputs.dropOffAddress);

        //contact
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 24, 'div', orderingAssertionsCilta.confirmation.contactName);
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 24, 'span', inputs.Lui);

        //number
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 25, 'div', orderingAssertionsCilta.confirmation.phoneNumber);
        //value
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 25, 'span', inputs.number3);

        //additional delivery notes
        translationHelpers.assertSectionChildElement('[data-testid=display-only]', 26, 'div', orderingAssertionsCilta.confirmation.additionalDeliveryNotes);
