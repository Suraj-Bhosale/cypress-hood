...
// Header
const collectionCiltaHeaderTranslations = () => {
...
// Phases
const collectionCiltaPhaseTranslations = () => {
...
export default {
	patientVerificationEmeaLc: (scope) => {
		...
		if (Cypress.env('runWithTranslations')) {
			...
			translationHelpers.assertSingleField('[data-testid=display-only] >> :nth(3)', scope.patientInformation.monthOfBirth)
			// Year of Birth
			translationHelpers.assertBlockLabel('[data-testid=display-only] >', {
				index: 8,
	...
	collectionBagIdentificationEmeaLc: () => {
		...
		if (Cypress.env('runWithTranslations')) { // title
			...
			cy.wait(3000)
			translationHelpers.assertBlockLabel('[data-test-id="enter-and-confirm-block"]>>> :nth(1), label', {
				index: 1,
		...
		// cy.get('[data-test-id="enter-and-confirm-block"] div ').first().should('contain', assertions.euUDNDescription);
	...
	collectionSummaryEmeaLc: (scope, therapy) => { // translations
		if (Cypress.env('runWithTranslations')) {
			...
			translationHelpers.assertSectionChildElement('[data-testid="display-only"]', 3, 'span', scope.patientInformation.monthOfBirth)
	...
	cryopreservationBagLabelsAndPackingInsert: (coi) => {
		if (Cypress.env('runWithTranslations')) {
			...
			// labels
			translationHelpers.assertBlockLabel('[data-test-id="label-step-row-description"]', {
				index: 0,
				label: collectionCiltaAssertion.cryopreservationBagLabelsAndPackingInsert.scanEnterDinSecBag
			})

			translationHelpers.assertSingleField('[data-testid="day-1-bag-1-udn-value"]', inputs.din)
		...
		cy.get('[data-testid="#/properties/shipping_checklist/properties/bag_1_product_volume-input"]').type('20')
		cy.get('[id="#/properties/data/properties/is_confirmed"]').click()
		actionButtonsHelper.clickActionButton(actionButtonsHelper.actionButtonKeys.PRIMARY, {
			apiAliases: ['@patchProcedureSteps', '@getProcedures']
		})
	...
	cryopreservationTransferProductToShipperEmeaLc: (coi, therapy, fromSite, toSite) => {
		...
		if (Cypress.env('runWithTranslations')) { // Title
			...
			// labels
			translationHelpers.assertSingleField('[data-testid=question-text-case_intact_1]', collectionCiltaAssertion.transferToShipper.questionCaseLcqr)

			translationHelpers.assertSingleField('[data-testid=question-text-temp_out_of_range_1]', collectionCiltaAssertion.transferToShipper.questionTempOutofRangeEmea)

			translationHelpers.assertSingleField('[data-test-id="label-step-row-description"]', collectionCiltaAssertion.transferToShipper.scanPackingInsertEmea)
	...
	// LCLP Cilta and EmeaLc
	collectionShippingSummaryEmeaLc: (scope) => {
		if (Cypress.env('runWithTranslations')) {
			...
			translationHelpers.assertSectionChildElement('[data-testid="display-only"]', 1, 'span', inputs.testCoe2);
			...
			translationHelpers.assertTxtFieldLayout(collectionCiltaAssertion.shipmentSummary.containerCaseLcEmea)
			translationHelpers.assertTxtFieldLayout(collectionCiltaAssertion.shipmentSummary.tempOutOfRangeLcEmea, "No")
