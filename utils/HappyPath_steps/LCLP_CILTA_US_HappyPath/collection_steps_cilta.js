...
export default {
	...
	cryopreservationBagLabelsAndPackingInsert: (coi) => {
		if (Cypress.env('runWithTranslations')) {
			...
			//translationHelpers.assertSingleField('[data-testid="#/properties/shipping_checklist/properties/collection_bag_label_1-input"]', inputs.din)
		...
		cy.get('[data-testid="#/properties/shipping_checklist/properties/volume_1-input"]').type('1')
		// cy.wait('@labelVerifications');
		cy.get('[id="#/properties/data/properties/is_confirmed"]').click()
		// common.ClickPrimaryActionButton();
		actionButtonsHelper.clickActionButton(actionButtonsHelper.actionButtonKeys.PRIMARY, {
	...
	cryopreservationSummary: (scope) => { // translations
		if (Cypress.env('runWithTranslations')) {
			...
			//translationHelpers.assertSectionChildElement('[data-testid=display-only]', 1, 'span', inputs.collectionSiteName2)
			...
			translationHelpers.assertSectionChildElement('[data-testid=display-only]', 2, 'span', `${scope.orderIdentifier
				}`)
			...
			// })
			translationHelpers.assertSingleField('[data-testid="txt-field-layout-Product Volume (Cells + CS10) in mL"]', collectionCiltaAssertion.cryopreservationSummary.productVolume)
	...
	cryopreservationTransferProductToShipper: (coi, therapy) => {
		...
		if (Cypress.env('runWithTranslations')) { // Title
			...
			// labels
			translationHelpers.assertSingleField('[data-testid=question-text-case_intact_1]', collectionCiltaAssertion.transferToShipper.questionCaseUSLclp)

			translationHelpers.assertSingleField('[data-testid=question-text-temp_out_of_range_1]', collectionCiltaAssertion.transferToShipper.questionTempOutofRangeCiltaLc)
		...
		cy.get(`[data-testid="ln-2-shipper-ship-bags-from-apheresis-site-to-manufacturing-site-us-lclp-cilta-1-input-field"]`).type(coi)
		cy.get(`[data-testid="ln-2-shipper-ship-bags-from-apheresis-site-to-manufacturing-site-us-lclp-cilta-1-action-trigger-button"]`).click()
	...
	cryopreservationShipmentChecklist: scope => {
		if (Cypress.env('runWithTranslations')) {
			...
			// labels
			translationHelpers.assertSingleField('[data-test-id="label-step-row-description"]', collectionCiltaAssertion.shipmentChecklist.enterAirWaybillUsLc)
	...
	collectionShippingSummaryEmeaLc: (scope, therapy) => {
		if (Cypress.env('runWithTranslations')) {
			...
			translationHelpers.assertTxtFieldLayout(collectionCiltaAssertion.shipmentSummary.containerCaseLcUs)
			translationHelpers.assertTxtFieldLayout(collectionCiltaAssertion.shipmentSummary.questionTempOutofRangeCiltaLc, "No")
