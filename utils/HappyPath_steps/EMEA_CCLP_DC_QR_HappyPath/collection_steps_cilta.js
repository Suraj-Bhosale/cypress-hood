...
export default {
	...
	patientVerificationEmeaCcDc: (therapy, scope) => {
		cy.log('Patient Verification')			
		// translations
		if (Cypress.env('runWithTranslations')) {
			if (!therapy.unique_id == '_emea_lcdc_qr')
	...
	collectionBagIdentificationEmeaCcDcQr: () => {
		cy.log('Bag Identification')
		// translations
		if (Cypress.env('runWithTranslations')) {
			...
			translationHelpers.assertBlockLabel('[data-test-id="enter-and-confirm-block"]>>> label', {
				index: 0,
				label: collectionCiltaAssertion.collectionBagIdentification.enter
			})
	...
	collectionSummaryEmeaCcDcQr: (scope) => { // translations
		if (Cypress.env('runWithTranslations')) { // title
			translationHelpers.assertPageTitles('[data-test-id="collection_collection_summary"]', 'h1', collectionCiltaAssertion.collectionSummary.title)
			...
			translationHelpers.assertSectionChildElement('[data-testid="display-only"]', 2, 'div', collectionCiltaAssertion.collectionSummary.dayOfBirth)
			translationHelpers.assertSectionChildElement('[data-testid="display-only"]', 2, 'span', inputs.dayOfBirthEmeaCcDcQr)

			translationHelpers.assertSectionChildElement('[data-testid="display-only"]', 3, 'div', collectionCiltaAssertion.collectionSummary.monthOfBirth)
			translationHelpers.assertSectionChildElement('[data-testid="display-only"]', 3, 'span', inputs.monthOfBirthEmeaCcDcQr)
	...
	transferProductToShipperCcDcQr: (coi, bag_identifier, therapy = 'emea-ccdc-qr', fromSite = 'apheresis-site', toSite = 'satellite-lab') => {
		cy.get(`[data-testid="ln-2-shipper-ship-bags-from-apheresis-site-to-satellite-lab-emea-ccdc-qr-input-field"]`).type(coi)
		cy.get(`[data-testid="ln-2-shipper-ship-bags-from-apheresis-site-to-satellite-lab-emea-ccdc-qr-action-trigger-button"]`).click()
	...
	//EmeaCcDcQr
	collectionShippingSummaryEmeaCcDcQr: (scope) => {
		if (Cypress.env('runWithTranslations')) {
