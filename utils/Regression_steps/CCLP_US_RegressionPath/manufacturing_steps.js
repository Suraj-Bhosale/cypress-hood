...
import m_steps from '../../../utils/HappyPath_steps/CCLP_US_HappyPath/manufacturing_steps';
import inputChecker from "../../shared_block_helpers/inputFieldCheckHelpers";
import regressionInput from '../../../fixtures/inputsRegression.json'
...
const getManufAirWayBill = (scope, shippingRow,coi) => {
...
export default {
  ...
  manufacturingVerifyShipper:  {
    previousHappyPathSteps: () => {
      m_steps.manufacturingCollectionSummary()
    },
    //C32540
    invalidCoi: () => {
      inputChecker.clearValueAndCheckForButton('[data-testid=ln-2-shipper-ship-bags-from-satellite-lab-to-manufacturing-site-cclp-input-field]','be.disabled')
     cy.get(`@coi`).then(coi => {
         inputChecker.scanAndVerifyCheck('ln-2-shipper-ship-bags-from-satellite-lab-to-manufacturing-site-cclp', `${coi}-FP-01`, 'not.be.visible', 'be.disabled');
         inputChecker.scanAndVerifyCheck('ln-2-shipper-ship-bags-from-satellite-lab-to-manufacturing-site-cclp', `${coi}-APH-01`, 'not.be.visible', 'be.disabled')
         inputChecker.scanAndVerifyCheck('ln-2-shipper-ship-bags-from-satellite-lab-to-manufacturing-site-cclp', `${coi}-PRC-01`, 'not.be.visible', 'be.disabled');
         inputChecker.scanAndVerifyCheck('ln-2-shipper-ship-bags-from-satellite-lab-to-manufacturing-site-cclp', `${coi}`, 'be.visible', 'not.be.disabled');
       });
      },
      //C32541
      checkForBackButton: () => {
  ...
  manufacturingShipmentReceiptChecklist:  {
    previousHappyPathSteps: () => {
      m_steps.manufacturingCollectionSummary();
      cy.get(`@coi`).then(coi => {
  ...
  manufacturingShipmentReceiptChecklistSummary:  {
    previousHappyPathSteps: () => {
      m_steps.manufacturingCollectionSummary();
      cy.get(`@coi`).then(coi => {
        m_steps.manufacturingVerifyShipper(coi, 'satellite-lab', '-cclp');
  ...
  manufacturingTransferProductToStorage:  {
  ...
  shippingManufacturing: {
   ...
   //	C40186	
    ...
    negSubjectNumberOnAirWaybillNotEntered: (scope) => {
      cy.get(`@coi`).then((coi) => {
      getManufAirWayBill(scope, 2,coi);
      })
      inputHelpers.inputSingleField('[data-testid="#/properties/shipping_checklist/properties/evo_last_4_digits-input"]', regressionInput.manufacturing.evoIsNumber);
      inputHelpers.inputSingleField('[data-testid="#/properties/shipping_checklist/properties/tamper_seal_number-input"]', regressionInput.manufacturing.tamperSealNumber);
      inputHelpers.clicker(['[data-testid="pass-button-investigational_product"]', '[data-testid="pass-button-cry_rack_label"]',
  ...
  checkStatusesOfManufacturingModule: (scope, therapy) => {
    //C40072
     cy.get(`@coi`).then((coi) => {
        ...
        m_steps.shippingManufacturing(2, scope, input.subjectNumberInput, input.evoLast4Digits, input.tamperSealNumber, coi);
