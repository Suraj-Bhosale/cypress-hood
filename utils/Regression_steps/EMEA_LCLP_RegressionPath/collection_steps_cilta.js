...
export default {
	...
	cryopreservationBagLabelsAndPackingInsert: (coi, therapy) => {
		if (Cypress.env('runWithTranslations')) {
		...
		if (therapy.unique_id == '_emea_lc_qr' || therapy.unique_id == '_emea_lc' || therapy.unique_id == '_emea_lcdc_qr' || therapy.unique_id == '_emea_lcdc')
	...
	cryopreservationSummary: (scope) => { // translations
		if (Cypress.env('runWithTranslations')) {
			...
			//TODO: Made these blocks conditionally visible. It will be visible only if corresponding data is entered in the previous steps.
			// translationHelpers.assertBlockLabel('[data-testid="txt-field-layout-Bag Identifier"]', {
			// 	index: 1,
