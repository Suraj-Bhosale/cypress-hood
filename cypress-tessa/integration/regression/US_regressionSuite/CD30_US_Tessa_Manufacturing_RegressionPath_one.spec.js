...
describe('US Tesscar Regression Path', () => {
  ...
  describe('Manufacturing Flow', () => {
    ...
    it('Lot Number' , () => {
      ...
      //C28121 [NEG] Verify the "Enter And Confirm" field with empty data.
      regressionManufacturingSteps.lotNumber.emptyData(scope);

      //C28122 [POS] Verify that while switching back from the "Collection Summary" page to the "Lot Number" page selected value should retain.
    ...
    it('Shipment Receipt Summary' , () => {
      ...
      regressionManufacturingSteps.shipmentReceiptSummaryUs.previousHappyPathSteps(scope);

      //C28092 [POS] Verify the "View Summary" button is disabled before signature and enabled after signature.
      regressionManufacturingSteps.shipmentReceiptSummaryUs.viewSummary();

      //C28093 [POS] Verify that after editing the page the "Next" button is disabled on the "Shipment Receipt Summary " page until re-signature is done.
      regressionManufacturingSteps.shipmentReceiptSummaryUs.editButton();
    });
  })
})
