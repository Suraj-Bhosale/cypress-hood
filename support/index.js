...
export default {
  ...
  ClickPrimaryActionButton: () => { // click on primary button
    cy.get('[data-testid="primary-button-action"]').should('be.visible').should('not.be.disabled').should('exist').click();
  ...
  doubleSignatureOrdering: (verifier) => {
    cy.get('[data-testid="verifier-sign-button"]').click();
    cy.get('[data-testid="password-field"]').type('password one two three');
    cy.waitForElementAndClick('.modal-buttons');
    cy.wait('@postSignature');
  ...
  ConfirmTreatmentInformation: data => {
    ...
    cy.get(`[data-testid="institution_id"] .select-content li[role="option"]:nth-child(${data.institution_select_index
      })`).then(() => { // close dropdown
        cy.waitForElementAndClick('[data-testid="institution_id"] .select');
      });
    ...
    cy.get(`[data-testid="physician_user_id"] .select-content li[role="option"]:nth-child(${data.prescriber_select_index
      })`).then(() => { // close dropdown
        cy.waitForElementAndClick('[data-testid="physician_user_id"] .select');
      });
  },
